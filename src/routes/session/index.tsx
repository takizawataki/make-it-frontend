...
import { browserName, mobileModel, osName } from 'react-device-detect';
...
import {
  ...
  GenerateAiReplyRequestUserAgent,
  UserUserResponse,
  getSession,
  getUserUserId,
...
function Session() {
  const { userAttributes, inviter, sessions } = useLoaderData({
    from: '/session/',
  });
  ...
  const sendMessage = async (message: string) => {
    try {
      ...
      let userAgent: GenerateAiReplyRequestUserAgent | undefined = undefined;

      // 新規セッション開始時、新しい sessionId を生成
      if (messages.length === 0) {
        currentSessionId = ulid();
      ...
      try {
        ...
        sessionIdForTitle = currentSessionId;
